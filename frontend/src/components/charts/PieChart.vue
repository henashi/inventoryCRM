...
const getSeriesData = () => {
  ...
  return props.data.labels.map((label, index) => ({
    ...
    label: activeDataIndex.value === index
      ? {
          show: false,
        }
      : undefined,
    labelLine: activeDataIndex.value === index
      ? {
          show: false,
        }
      : undefined,
...
const getChartGeometry = () => {
  ...
  const centerY = typeof center[1] === 'string' && center[1].endsWith('%')
  ...
  const outerRadius = typeof radius === 'string' && radius.endsWith('%')
