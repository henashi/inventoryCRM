  ...
  <div ref="containerRef">
    ...
    <div v-if="isOpen" class="fab-popup" :style="popupStyle">
      ...
      <div class="fab-popup-footer">
        ...
        />
        <a-button type="primary" size="small" :loading="fabLoading" @click="fabSend(fabInput)"
          >发送</a-button
        >
      </div>
    </div>
...
// @ts-nocheck
    ...
    if (e.key === 'ArrowUp') {
      ...
      const idx =
        fabHistoryIndex.value === -1
          ? fabHistory.value.length - 1
          : Math.max(0, fabHistoryIndex.value - 1)
  ...
  const fabSend = async (text: string) => {
    ...
    try {
      const history = fabMessages.value
        .slice(-10)
