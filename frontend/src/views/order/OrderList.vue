        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'amount'">
            <div>
              ...
              <div v-if="record.discount > 0" class="meta-cell">
                优惠 ¥{{ record.discount.toFixed(2) }}
              </div>
            </div>
          </template>
        ...
        <a-form-item label="客户" v-bind="validateInfos.customerId">
          ...
          >
            <a-select-option v-for="c in customerOptions" :key="c.id" :value="c.id"
              >{{ c.name }}（{{ c.phone }}）</a-select-option
            >
          </a-select>
        </a-form-item>
          ...
          <a-row :gutter="8" type="flex" align="middle">
            ...
            <a-col :span="5">
              <a-form-item :label="idx === 0 ? '金额' : ''" style="margin-bottom: 0">
                <a-input-number
                  v-model:value="item.totalAmount"
                  :min="0"
                  :precision="2"
                  prefix="¥"
                  style="width: 100%"
...
<script setup lang="ts">
  ...
  function loadData(page = 0) {
    loading.value = true
    orderApi
      .list({ page, size: pagination.value.pageSize })
      .then((res) => {
