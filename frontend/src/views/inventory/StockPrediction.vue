      ...
      >
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'daysToEmpty'">
            <a-tag
              v-if="record.estimatedDaysToEmpty >= 0"
              :color="getDaysColor(record.estimatedDaysToEmpty)"
            >
          ...
          <template v-if="column.key === 'action'">
            <a-button type="link" size="small" @click="goToStockIn(record)"> 补货 </a-button>
          </template>
        </template>
            ...
            <div class="trend-header">
              ...
              <div class="trend-legend">
                <span class="legend-item"
                  ><span class="legend-dot bar-color"></span> 每日出库量</span
                >
                <span class="legend-item"
                  ><span class="legend-dot avg7-color"></span> 近7日均线 ({{
                    record.avgDailyOut7d?.toFixed(1) ?? '-'
                  }})</span
                >
                <span class="legend-item"
                  ><span class="legend-dot avg30-color"></span> 近30日均线 ({{
                    record.avgDailyOut30d?.toFixed(1) ?? '-'
                  }})</span
                >
                <span class="legend-item"
                  ><span class="legend-dot safe-color"></span> 安全库存线 ({{
                    record.safeStock
                  }})</span
                >
              </div>
...
<script setup lang="ts">
  ...
  const handleRunPrediction = async () => {
    ...
    try {
      ...
      message.success(
        `全量预测完成！分析 ${result.totalPredicted} 个商品，${result.dangerCount} 个高危，耗时 ${result.executionTimeMs}ms`,
      )
      await loadData()
    } catch {
