        ...
        <div class="search-grid">
          ...
          <div class="search-field search-field-checkbox">
            <a-form-item label="筛选项">
              <a-checkbox v-model:checked="searchForm.lowStockOnly" @change="handleSearch"
                >仅看低库存</a-checkbox
              >
            </a-form-item>
          </div>
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.dataIndex === 'currentStock'">
            <a-tag :color="getStockColor(record)"
              >{{ record.currentStock }} {{ record.unit }}</a-tag
            >
          </template>
          ...
          <template v-else-if="column.dataIndex === 'alert'">
            <a-tag v-if="record.lowStock || record.outOfStock" color="red">{{
              record.alertReason || '低库存'
            }}</a-tag>
            <span v-else>正常</span>
          </template>
...
<script setup lang="ts">
  ...
  const handleExport = async () => {
    try {
      ...
      await inventoryStore.exportInventories(
        getQueryParams(inventoryStore.pagination.page - 1, inventoryStore.pagination.size),
      )
