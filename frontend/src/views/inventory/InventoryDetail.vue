      ...
      <a-col :xs="24" :xl="14">
        <a-card :loading="inventoryStore.isLoading" title="基础信息">
          <a-descriptions v-if="inventory" :column="2" bordered>
            ...
            <a-descriptions-item label="当前库存">{{ inventory.currentStock }} {{ inventory.unit }}</a-descriptions-item>
            <a-descriptions-item label="安全库存">{{ inventory.safeStock }} {{ inventory.unit }}</a-descriptions-item>
            <a-descriptions-item label="最大库存">{{ inventory.maxStock }} {{ inventory.unit }}</a-descriptions-item>
            <a-descriptions-item label="状态">
              <a-tag :color="inventory.status === 1 ? 'green' : 'default'">
                {{ inventory.status === 1 ? '正常' : '停用' }}
              </a-tag>
            </a-descriptions-item>
            <a-descriptions-item label="最近更新时间" :span="2">{{ formatDateTime(inventory.lastUpdateTime) }}</a-descriptions-item>
          </a-descriptions>
        </a-card>
      </a-col>
...
const changeColumns = [
  ...
  { title: '库存变化', dataIndex: 'afterQuantity', key: 'afterQuantity', customRender: ({ record }: { record: { beforeQuantity: number; afterQuantity: number } }) => `${record.beforeQuantity} → ${record.afterQuantity}` },
