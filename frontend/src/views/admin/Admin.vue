      ...
      <a-col :xs="24" :xl="12">
        <a-card title="库存与日志提醒" class="section-card" :loading="loading">
          ...
          <div v-else class="warning-list">
            <div v-for="product in lowStockProducts" :key="product.id || product.code" class="warning-item">
              <div>
                <div class="warning-title">{{ product.name }}</div>
                <div class="warning-desc">{{ product.code }} · 当前 {{ product.currentStock }} {{ product.unit }} / 安全库存 {{ product.safeStock }} {{ product.unit }}</div>
              </div>
        ...
        <a-card title="操作日志预览" class="section-card" :loading="loading">
          <div class="runtime-list">
            ...
            <div class="runtime-item">
              ...
              <span>{{ operationLogSummary.successCount }} / {{ operationLogSummary.failureCount }}</span>
            </div>
            <div class="runtime-item">
          ...
          <div v-else class="log-list">
            <div v-for="item in recentOperationLogs" :key="item.id" class="log-item">
              <div>
                ...
                <div class="log-desc">{{ item.operator || '系统' }} · {{ item.description || item.operationType }} · {{ formatDateTime(item.operationTime) }}</div>
              </div>
...
const loadOverview = async () => {
  ...
  const results = await Promise.allSettled([
    ...
    operationLogApi.searchLogs({ page: 0, size: 6 }) as Promise<{ content: OperationLogRecord[] }>,
