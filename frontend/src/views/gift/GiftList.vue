    ...
    >
      ...
      >
        ...
        <a-form-item label="礼品类型" name="type">
          <a-select v-model:value="formState.type" placeholder="请选择礼品类型" allow-clear>
        ...
        <a-form-item label="状态类型" name="status">
          <a-select v-model:value="formState.status" placeholder="请选择状态类型" allow-clear>
        ...
        <a-form-item label="限制数量" name="limitPerPerson">
          <a-input-number
            v-model:value="formState.limitPerPerson"
    ...
    <a-drawer v-model:open="detailVisible" title="礼品详情" width="520" :destroy-on-close="true">
      <a-spin :spinning="detailLoading">
        ...
        <a-descriptions v-else :column="1" bordered size="small">
          ...
          <a-descriptions-item label="礼品类型">{{
            getTypeText(detailGift.type)
          }}</a-descriptions-item>
          <a-descriptions-item label="礼品状态">{{
            getStatusText(detailGift.status)
          }}</a-descriptions-item>
          <a-descriptions-item label="关联商品">{{
            detailGift.productName || '无关联商品'
          }}</a-descriptions-item>
          <a-descriptions-item label="限额开关">{{
            detailGift.limitEnabled ? '开启' : '关闭'
          }}</a-descriptions-item>
          <a-descriptions-item label="每人限额">{{
            detailGift.limitPerPerson ?? '--'
          }}</a-descriptions-item>
          <a-descriptions-item label="礼品描述">{{
            detailGift.description || '--'
          }}</a-descriptions-item>
          <a-descriptions-item label="备注">{{ detailGift.remark || '--' }}</a-descriptions-item>
          <a-descriptions-item label="创建时间">{{
            formatDateTime(detailGift.createdTime)
          }}</a-descriptions-item>
          <a-descriptions-item label="更新时间">{{
            formatDateTime(detailGift.updatedTime)
          }}</a-descriptions-item>
        </a-descriptions>
      </a-spin>
    </a-drawer>
...
<script lang="ts" setup>
  ...
  type GiftFormState = Omit<
    GiftCreateDTO,
    'type' | 'limitPerPerson' | 'startTime' | 'endTime' | 'productId'
  > & {
