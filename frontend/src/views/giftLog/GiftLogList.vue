    ...
    >
      ...
      >
        ...
        <a-form-item label="发放说明" name="issueNotes">
          <a-textarea
            v-model:value="formState.issueNotes"
            placeholder="请输入发放说明"
            :maxlength="50"
            show-count
          />
...
<script lang="ts" setup>
  ...
  const syncFilterCustomerName = () => {
    const matchedCustomer = customers.value.find(
      (customer) => customer.id === giftLogFilter.customerId,
    )
  ...
  const loadCustomersForOptions = async () => {
    try {
      const response = (await customerApi.getCustomers({ page: 0, size: 100 })) as unknown as {
        content: Customer[]
      }
