  ...
  <div class="customer-detail-page">
    ...
    <a-card>
      <a-spin :spinning="pageLoading">
        <a-result v-if="pageError" status="error" title="客户详情加载失败" :sub-title="pageError">
          <template #extra>
            <a-space>
              <a-button @click="goBack">返回</a-button>
            ...
            <div class="hero-main">
              ...
              <div>
                ...
                <div class="hero-meta">
                  ...
                  <span
                    >注册于 {{ formatDateTime(customer.registeredAt || customer.createdAt) }}</span
                  >
                </div>
              </div>
            </div>
              ...
              <a-card title="档案信息" class="section-card">
                <a-descriptions :column="2" bordered>
                  ...
                  <a-descriptions-item label="注册时间">{{
                    formatDateTime(customer.registeredAt || customer.createdAt)
                  }}</a-descriptions-item>
              ...
              <a-card title="最近礼品记录" class="section-card">
                ...
                <a-spin :spinning="giftLogLoading">
                  ...
                  <div v-else class="gift-log-list">
                    <div
                      v-for="giftLog in relatedGiftLogs"
                      :key="giftLog.id || `${giftLog.giftId}-${giftLog.createdTime}`"
                      class="gift-log-item"
                    >
                      <div>
                        ...
                        <div class="gift-log-meta">
                          ...
                          <span
                            >发放时间
                            {{ formatDateTime(giftLog.issuedAt || giftLog.createdTime) }}</span
                          >
...
<script setup lang="ts">
  ...
  const issuedGiftCount = computed(
    () => relatedGiftLogs.value.filter((item) => item.status === 'ISSUED').length,
  )
  const pendingGiftCount = computed(
    () => relatedGiftLogs.value.filter((item) => item.status === 'PENDING').length,
  )
