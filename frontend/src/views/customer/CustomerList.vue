<template>
  <div class="customer-list-page">
        ...
        <template #bodyCell="{ column, record }">
            ...
            <div class="customer-name">
              ...
              <div class="name-info">
                ...
                <div class="customer-tags">
                  ...
                  <a-tag v-if="record.referrerId" color="blue" size="small"> 转介绍 </a-tag>
                </div>
              </div>
            </div>
          </template>
          ...
          <template v-else-if="column.dataIndex === 'actions'">
            <a-space size="small">
              ...
              <a-button v-else type="link" @click="handleEnable(record)">
                <check-outlined />
                启用
              </a-button>
              <a-button
      ...
      <div class="import-section">
        <div class="import-header">
          <div>
            ...
            <div class="import-subtitle">
              导入结果会返回失败明细，成功数据将自动刷新当前列表与统计。
            </div>
          </div>
          <a-space>
            <a-button size="small" :disabled="!customerImportMeta" @click="copyImportTemplateHeader"
              >复制模板表头</a-button
            >
        ...
        <a-card v-if="customerImportMeta" size="small" class="import-meta-card">
          <div class="meta-row">
            ...
            <a-space wrap>
              <a-tag v-for="field in customerImportMeta.templateFields" :key="field">{{
                field
              }}</a-tag>
            </a-space>
          </div>
          <div class="meta-row">
            ...
            <a-space wrap>
              <a-tag v-for="field in customerImportMeta.requiredFields" :key="field" color="red">{{
                field
              }}</a-tag>
            </a-space>
          </div>
    ...
    >
      ...
      >
        ...
        <a-form-item label="礼品等级" name="giftLevel">
          <a-select v-model:value="formState.giftLevel" placeholder="请选择礼品等级">
            <a-select-option :value="0">普通客户</a-select-option>
            <a-select-option :value="1">等级1</a-select-option>
            <a-select-option :value="2">等级2</a-select-option>
            <a-select-option :value="3">等级3</a-select-option>
    ...
    <a-drawer v-model:open="drawerVisible" title="客户详情" width="400" placement="right">
      <template v-if="currentCustomer">
        <div class="customer-detail">
          <div class="detail-header">
            <a-avatar
              :size="64"
              :style="{ backgroundColor: getCustomerColor(currentCustomer.name) }"
            >
          ...
          <div class="detail-actions">
            <a-button type="primary" @click="handleEdit(currentCustomer)" block> 编辑 </a-button>
          </div>
        </div>
      </template>
    </a-drawer>
...
<script setup lang="ts">
  ...
  const customerImportMeta = computed(() =>
    buildCustomerImportMeta(
      customerImportResult.value || customerImportTemplate.value || undefined,
    ),
  )
  ...
  const handleEdit = async (record: Customer) => {
    ...
    if (
      formState.referrerId &&
      !referrerList.value.find((item) => item.id === formState.referrerId)
    ) {
  ...
  const handleBatchExport = () => {
    const records = customers.value.filter(
      (customer) => customer.id && selectedRowKeys.value.includes(customer.id),
    )
    if (!records.length) {
  ...
  const submitImport = async () => {
    ...
    try {
      ...
      const result = (await customerApi.importCustomers(
