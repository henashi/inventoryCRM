...
export const aiApi = {
  ...
  /** 查询预测概览统计 */
  getPredictionSummary: () => request.get<PredictionSummary>('/ai/predictions/summary'),
  ...
  /** 手动触发全量预测 */
  runPrediction: () => request.post<AiRunResult>('/ai/predictions/run'),

  // ===== 客户评分 & 礼品推荐 =====

  /** 查询客户评分列表（分页） */
  getCustomerScores: (params?: { page?: number; size?: number }) =>
    request.get<{ content: CustomerScore[]; totalElements: number; totalPages: number }>(
      '/ai/customers/scores',
      { params },
    ),
  ...
  /** 查询未来7天生日的客户 */
  getUpcomingBirthdayCustomers: () => request.get<{ id: number; name: string }[]>('/ai/customers/birthday-upcoming'),
